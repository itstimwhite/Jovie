...
export function ThemeToggle() {
  ...
  if (!mounted) {
    return (
      <div className="h-10 w-[140px] animate-pulse rounded-full bg-gray-200 dark:bg-gray-700" />
    );
  }
  ...
  return (
    <div className="grid grid-cols-1 max-sm:hidden">
      <div className="relative z-0 inline-grid grid-cols-3 gap-0.5 rounded-full bg-gray-950/5 p-0.75 text-gray-950 dark:bg-white/10 dark:text-white">
        <RadioGroup value={theme} onChange={setTheme} className="contents">
          {/* System theme option */}
          <RadioOption value="system">
    ...
    <RadioGroup.Option
      ...
      className={({ checked }) =>
        clsx(
          ...
          checked && 
          'bg-white ring inset-ring ring-gray-950/10 inset-ring-white/10 dark:bg-gray-600 dark:text-white dark:ring-transparent'
        )
      }
    >
      {({ checked }) => (
        <>
          <span className="sr-only">
            {value === 'system' ? 'System theme' : value === 'light' ? 'Light theme' : 'Dark theme'}
          </span>
          {children}
        </>
      )}
