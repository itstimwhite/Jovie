...
for (const line of lines) {
    ...
    const id = crypto.createHash("sha1").update(`${title}|${currentSection || ''}`).digest("hex").slice(0, 10);
    const labels = new Set(["mvp", "agent:copilot", sectionLabelMap.get(currentSection) || "area:unsorted"]);
    if (state === "/") labels.add("status:in-progress");
    tasks.push({ id, title, state, section: currentSection, labels: Array.from(labels) });
}

async function ensureLabel(name, color, description) {
    try { await octo.rest.issues.getLabel({ owner, repo, name }); }
    catch {
        await octo.rest.issues.createLabel({ owner, repo, name, color, description });
    }
}
...
async function assignCopilot(issue_number) {
    try {
        await octo.rest.issues.addAssignees({ owner, repo, issue_number, assignees: [copilotAssignee] });
    } catch (e) {
        console.warn(`Assignee '${copilotAssignee}' failed for #${issue_number}: ${e.status || ''} ${e.message || e}`);
    }
}
...
for (const t of tasks) {
    ...
    if (!found) {
        ...
        created++;
        if (t.state === "x") {
            await octo.rest.issues.update({ owner, repo, issue_number: res.data.number, state: "closed" });
            closed++;
        }
        continue;
    }
    ...
    // Update existing
    await octo.rest.issues.update({ owner, repo, issue_number: found.number, title: t.title, body });
    await octo.rest.issues.setLabels({ owner, repo, issue_number: found.number, labels: t.labels });
    updated++;
    ...
    if (!shouldClose) {
        const alreadyAssigned = (found.assignees || []).some(a => a.login === copilotAssignee);
        if (!alreadyAssigned) await assignCopilot(found.number);
    }
}

console.log(`Synced ${tasks.length} tasks from ${planPath} (created:${created}, updated:${updated}, closed:${closed}, reopened:${reopened})`);