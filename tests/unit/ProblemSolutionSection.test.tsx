...
describe('ProblemSolutionSection', () => {
  it('renders unified section with problem and solution content', () => {
    ...
    expect(screen.getByText('Your bio link is a speed bump.')).toBeInTheDocument();
    expect(screen.getByText('We built the off-ramp.')).toBeInTheDocument();
    
    // Check problem description
    expect(screen.getByText(/Every extra tap taxes attention/)).toBeInTheDocument();
    
    // Check solution description
    expect(screen.getByText(/Jovie ships a locked, elite artist page/)).toBeInTheDocument();
  });
  ...
  it('renders Linear-inspired CTA button with proper styling', () => {
    ...
    expect(ctaButton).toHaveClass('inline-flex', 'items-center', 'justify-center');
  ...
  it('tracks analytics when CTA button is clicked', () => {
    ...
    expect(track).toHaveBeenCalledWith('claim_handle_click', { section: 'problem-solution' });
  });
  ...
  it('has proper accessibility attributes', () => {
    ...
    expect(section).toHaveAttribute('aria-labelledby', 'problem-solution-heading');
    
    const heading = screen.getByRole('heading', { name: /Your bio link is a speed bump/ });
